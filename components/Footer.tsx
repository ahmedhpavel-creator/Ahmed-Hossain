...
import { ORGANIZATION_INFO, TRANSLATIONS, LOGO_URL } from '../constants';
...
const Footer: React.FC = () => {
  ...
  const [socialLinks, setSocialLinks] = useState({ facebook: '', youtube: '', twitter: '' });
    ...
    <footer className="bg-gray-900 text-white relative no-print">
      ...
      <div className="container mx-auto px-4 pt-16 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
               {socialLinks.facebook && (
                 <a href={socialLinks.facebook} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-blue-600 transition-colors text-gray-400 hover:text-white">
                   <Facebook size={16} />
                 </a>
               )}
               {socialLinks.youtube && (
                 <a href={socialLinks.youtube} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-red-600 transition-colors text-gray-400 hover:text-white">
                   <Youtube size={16} />
                 </a>
               )}
               {socialLinks.twitter && (
                 <a href={socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-sky-500 transition-colors text-gray-400 hover:text-white">
                   <Twitter size={16} />
                 </a>
               )}
            </div>
          ...
          {/* Quick Links */}
          <div>
             <h4 className="text-lg font-bold mb-6 text-white">{lang === 'en' ? 'Quick Links' : 'দ্রুত লিঙ্ক'}</h4>
             <ul className="space-y-3">
               ...
               <li>
                 <Link to="/admin" className="text-gray-500 hover:text-gray-300 transition-colors text-sm flex items-center gap-2">
          ...
          {/* Contact */}
          <div>
            <h4 className="text-lg font-bold mb-6 text-white">{lang === 'en' ? 'Contact Us' : 'যোগাযোগ'}</h4>
